...
function fnNavigation() {
	...
	fnBlockRegions = new NBlock(ST_REGION, ST_REGION,
	{
	    dataURL: locationFloodingData,
	    fields:[
	...
	fnBlockBreaches = new NBlock(ST_BREACH ,ST_BREACH ,
	{
	    dataURL:locationFloodingData,
	    fields:[
...
/******************* Fourth step  *********************/
	...
	function check_step_two() {
		//to do: add check
		if (selectInundationModel.getSelectedRecord() && (selectExtwaterModel.isDisabled() || selectExtwaterModel.getSelectedRecord())) {
			set_step_three();
		}
	}
