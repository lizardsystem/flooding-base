...
def search(context, search_fields):
    ...
    <type>, 'options': {'<option>': <value>}, 'fields': ((<name>, <value>), (<name>, <value>), ...)], ...}  
...
def search_form(context, search_field='q', search_value='', hidden_fields={}):
    ...
    hidden_fields: optional: dictionaries of {fieldname: fieldvalue}. removes key entry with search_field
    """

    fields = hidden_fields.copy()
    if fields.has_key(search_field):
...
def digg_paginator(context, digg_fields={}):
    ...
    if paginator is None, or page is None, or nrof pages == 1, then is_paginated = False
    extra_fields is a dictionary with {field_name: field_value}, removes 'page_nr' and 'items_per_page'
    ...
    if (is_paginated):
        ...
        if (paginator.num_pages <= LEADING_PAGE_RANGE_DISPLAYED):
            ...
            page_numbers = [n for n in range(page.number - ADJACENT_PAGES, page.number + ADJACENT_PAGES + 1) if n > 0 and n <= paginator.num_pages]
            pages_outside_leading_range = [n + paginator.num_pages for n in range(0, -NUM_PAGES_OUTSIDE_RANGE, -1)]
            pages_outside_trailing_range = [n + 1 for n in range(0, NUM_PAGES_OUTSIDE_RANGE)]
        ...
        if extra_fields.has_key('page_nr'):
            del extra_fields['page_nr']
        if extra_fields.has_key('items_per_page'):
...
def filter_form(context, choices, output_fieldname, current_value, extra_fields, 
                name=""):
    """renders a choice menu (to filter lists)
    
    choices: tuple of (value, name)
    output_name: name of GET parameter
    extra_fields: list of dictionaries [{'name': name, 'value': value}, ...] (removes entry
      with output_fieldname)

    """
    fields = extra_fields.copy()
    if fields.has_key(output_fieldname):
...
def filters(entries, extra_fields):
    ...
    for entry in entries:
        if fields.has_key(entry['output_name']):
...
def objecttable(name, columns, data, sort):
    ...
    data=list of rows, where row = [{'url': optional url, 'value': value},... ] --> see below

    sort=current sort value


    data row dictionary has the following possible keys:
    url: displays a href around value and icon, (icon is not showed when urlpost has a value)
    urlpost: displays a form, be sure that icon has a value
    value: text to display
    icon: iconname, without extension .png. e.g. edit, edit_disabled, delete, delete_disabled
